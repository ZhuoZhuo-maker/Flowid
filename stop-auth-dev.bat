...
echo [STEP 1/3] Stop auth server process on port %PORT%...
...
if "%EXIT_CODE%"=="0" (
  echo [DONE] stop-auth-dev finished successfully.
) else (
  echo [DONE] stop-auth-dev finished with error code %EXIT_CODE%.
)
exit /b %EXIT_CODE%
